...
if (-not $Destination) { $Destination = Join-Path $scriptRoot '..\Stl2StepFreeCAD\bin\windows-x86_64' }
...
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) "stl2step-freecad-$([Guid]::NewGuid())"
